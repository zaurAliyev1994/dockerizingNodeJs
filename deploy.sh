...
RUNNING_TASK=$(aws ecs list-tasks --cluster $CLUSTER | jq -r '.taskArns | .[0]')
aws ecs stop-task --task $RUNNING_TASK  --cluster $CLUSTER
aws ecs start-task --task-definition $TASK  --cluster $CLUSTER --container-instances $CONTAINER_INSTANCE
